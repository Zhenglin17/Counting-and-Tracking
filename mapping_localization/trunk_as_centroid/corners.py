...
def shi_tomasi(image, increase_points = 0):
...
#    Python: cv2.goodFeaturesToTrack(image, maxCorners, qualityLevel, minDistance[, corners[, mask[, blockSize[, useHarrisDetector[, k]]]]])
    if increase_points == 0:
        # corners_img = cv2.goodFeaturesToTrack(gray_img,10000,0.005,10)
        corners_img = cv2.goodFeaturesToTrack(gray_img,10000,0.0075,10)
    elif increase_points == 1:
        corners_img = cv2.goodFeaturesToTrack(gray_img,10000,0.0025,10)
    elif increase_points == 2:
        corners_img = cv2.goodFeaturesToTrack(gray_img,10000,0.001,10)
