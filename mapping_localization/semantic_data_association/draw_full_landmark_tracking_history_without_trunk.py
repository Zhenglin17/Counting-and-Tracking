...
def draw_landmark_counted_fruits_without_trunk(start_idx_2D_tracking,start_idx_diff, end_idx,inp_format,pred_format,data_dir_pre, min_age = 3, rot_trans_dict = None):
...
# ****************************************************************replacement ends here**********************************************************************************************************************************
    ...
    for i in range(start_idx_diff, end_idx):
            ...
            for track_dupflag in track_last_observe_fr_num_dict[i]:
                ...
                if track.age >= age_threshold:
                    ...
                    if dupflag:
                        ...
                        total_counts_all += 1
                        if True: # os.path.exists(binary_mask_name):
                            if True:#check_bbox_inside_mask(mask, bbox):
        ...
        draw_text_mask(bgr, '', 'Green: valid tracks (counted upon disappearing)', (10, h - 20), scale=1.5,color=Colors.cyan)

        draw_text_mask(bgr_vis, '', 'Red: fruits being counted in current frame.', (10, h - 70), scale=1.5,
            color=Colors.cyan)
        draw_text_mask(bgr_vis, '', 'Green: valid tracks. Blue: detections not yet considered as valid.', (10, h - 20), scale=1.5,
            color=Colors.cyan)
...
if __name__ == '__main__':
    ...
    draw_landmark_counted_fruits_without_trunk(start_idx_2D_tracking = start_idx,start_idx_diff = 0,end_idx = end_idx,
