...
class FruitTracker(object):
    ...
    # TODO:
    def __init__(self, min_age=3, win_size= np.array([25, 30]), max_level=7, fruits_moving_direction = 0, min_movement_threshold = 10,
                 state_cov=(3, 1, 3, 1), proc_cov=(9, 3, 9, 3),
                 flow_cov=(6, 2), bbox_cov=(1, 0.3), vel_cov=(3, 0.9), margin_pixels = COUNT_CUTOUT_PADDING):
    ...
    #      flow_cov=(2, 1), bbox_cov=(0.2, 0.1), vel_cov=(0.2, 0.1), margin_pixels = COUNT_CUTOUT_PADDING):
        ...
        self.fruits_moving_direction = fruits_moving_direction
        self.init_flow = np.array([fruits_moving_direction*10, 0])
        self.init_flow_avg = np.array([fruits_moving_direction*10, 0])
        self.init_flow_dir = np.array([fruits_moving_direction, 0])
        ...
        self.avg_bbox_area = self.win_size[0]*self.win_size[1]
    ...
    def add_new_tracks(self, tracks, fruits):
        ...
        """
        current_avg_bbox_area = 0
        for fruit in fruits:
            current_avg_bbox_area += bbox_area(fruit) / float(len(fruits))
        self.avg_bbox_area = 0.5*self.avg_bbox_area + 0.5*current_avg_bbox_area
        print('average_bbox_area is',self.avg_bbox_area)

        for fruit in fruits:
            track = FruitTrack(fruit, self.init_flow, self.state_cov,
                               self.proc_cov, self.avg_bbox_area)
            tracks.append(track)
    ...
    def track(self, image, fruits, bw, prev_bgr):
        ...
        if self.tracks != []:
            prev_pts = [t.prev_pos for t in self.tracks if bbox_area(t.bbox)]# > 0.2*self.avg_bbox_area]
            init_pts = [t.pos for t in self.tracks if bbox_area(t.bbox)]# > 0.2*self.avg_bbox_area]

            # prev_pts2 = [t.prev_pos for t in self.tracks if bbox_area(t.bbox) <= 0.2*self.avg_bbox_area]
            # init_pts2 = [t.pos for t in self.tracks if bbox_area(t.bbox) <= 0.2*self.avg_bbox_area]
            #
            # if len(prev_pts2) < 2:
            #     prev_pts.extend(copy.deepcopy(prev_pts2))
            #     init_pts.extend(copy.deepcopy((init_pts2))
            #     prev_pts2 = []
            #     init_pts = []
            ...
            curr_pts, status = calc_optical_flow(self.prev_gray, gray,
                                                 ...
                                                 self.max_level)
            # if prev_pts2 != []:
            #     curr_pts2, status2 = calc_optical_flow(self.prev_gray, gray,
            #                              prev_pts2, init_pts2,
            #                              (self.win_size*0.2).astype(int),
            #                              self.max_level)
            #
            #     curr_pts = np.append(curr_pts,curr_pts2,0)
            #     status = np.append(status,status2,0)
            #     prev_pts.extend(prev_pts2)
            #     init_pts.extend(init_pts2)
        ...
        if self.tracks != []:
            ...
            self.init_flow_avg = 0.5*self.init_flow + 0.5*self.init_flow_avg
            norm_avg_flow = np.linalg.norm(self.init_flow)
            dir_avg_flow = self.init_flow / max(norm_avg_flow,1e-5)
            if norm_avg_flow > self.min_movement_threshold:
                self.init_flow_dir = 0.5*self.init_flow_dir + 0.5*dir_avg_flow

            self.init_flow_dir = self.init_flow_dir / max(np.linalg.norm(self.init_flow_dir),1e-5)
            ...
            if self.prev_detection_exist == 1:
                if fruits.shape[0] != 0:
                    if len(curr_pts.shape) == 1:
                        ...
                        for track, point, stat in zip(self.tracks, curr_pts, status):
                            if stat:
                                new_bboxes = shift_bbox(track.bbox, point)
                                KLT_bboxes.append(new_bboxes)
                                KLT_tracks.append(track)
        ...
        if self.tracks != [] and self.prev_detection_exist == 1:
            updated_tracks= self.update_tracks(gray,  fruits_list, matched_KLT_points)
            lost_tracks = []
            self.prev_detection_exist = 1
        ...
        # if self.vis:
            if not draw_two_frame_only:
                for track in self.tracks:
                    color_value = 255
                    draw_line(self.disp_bgr, track.hist, color=[color_value,color_value,color_value], thickness = 1, dash_gap=15)
                    draw_line(self.disp_bw, track.hist, color=[color_value,color_value,color_value], thickness = 1, dash_gap=15)
                    draw_line(self.disp_bw, track.hist, color=[color_value,color_value,color_value], thickness = 1, dash_gap=15)
        ...
        if self.vis:
            h, w = np.shape(bw)
            # draw total count
            draw_text_mask(self.disp_bgr, self.total_counts, 'Total count:', (10, h - 50), scale = 1, color = Colors.cyan)
    ...
    def update_tracks(self, gray, fruits_list, matched_KLT_points):
        ...
        if len(curr_pts) == 1:
            ...
            for track, KLT_point, prev_point, FCN_bbox in zip(self.tracks, curr_pts, prev_pts, fruits_list):
                vel_dir = self.init_flow_dir
                vel_mag = max(np.linalg.norm(KLT_point - prev_point), 1e-5)
                vel = vel_mag * vel_dir
                track.para_update_bbox(KLT_point, FCN_bbox, vel, self.flow_cov, self.bbox_cov, self.vel_cov)
        ...
        # for track, fruit in zip(self.tracks, fruits_list):
        #     track.correct_bbox(fruit, self.bbox_cov)
        # # TODO: NEWNEW update velocity
        # for track, point, prev_point, stat in zip(self.tracks, curr_pts, prev_pts, status):
        #     if stat:
        #         vel_dir = self.init_flow / np.linalg.norm(self.init_flow)
    ...
    def match_tracks(self, bboxes, tracks, fruits, estimated_KLT_points):
        ...
        if tracks == []:
            ...
            if len(bboxes) < 30:
                print('too few bboxes, choose a larger Hungarian unassigned cost!')
                match_inds, lost_inds, new_inds = hungarian_assignment(cost, unassigned_cost= 2.7 - (len(bboxes)) * 0.05)
            else:
       ...
       # match_inds, lost_inds, new_inds = hungarian_assignment(cost)
        ...
        for match in match_inds:
            ...
            # TODO: no longer use this to aviod noise
    ...
    def count_in_tracks(self, tracks, prev_bgr):
        ...
        frame_bboxes = []
        frame_bbox_imgs = []
        counted_fruits_and_last_frame = []
        for t in tracks:
            if t.age >= self.min_age:
                frame_bboxes.append(t.bbox)
                # y1 = t.bbox[1]
                # y2 = t.bbox[1]+t.bbox[3]
                # x1 = t.bbox[0]
                # x2 = t.bbox[0]+t.bbox[2]
                # bbox_img_temp = self.disp_bgr[y1:y2,x1:x2,:]
                # TODO: get the bbox position in previous frame!(for counting)
                bbox_pre_fr = t.bbox
                bbox_pre_fr[0] += t.prev_pos[0] - t.pos[0]
                bbox_pre_fr[1] += t.prev_pos[1] - t.pos[1]
                #TODO: add margin to bounding boxes so that the bboxes are more complete
                bbox_pre_fr[0] -= self.margin_pixels
                bbox_pre_fr[1] -= self.margin_pixels
                bbox_pre_fr[2] += 2*self.margin_pixels
                bbox_pre_fr[3] += 2*self.margin_pixels

                # print ('pos0,pos2',bbox_pre_fr[0],bbox_pre_fr[2],'\n')
                bbox_img_temp = extract_bbox(prev_bgr, bbox_pre_fr, copy=False)

                #TODO: here save the counted fruits and last frame for our localization!!!! bbox_pre_frame is (x, y, w, h)with margin_pixels and t.prev_pos is center position of bbox [xc,yc]
                #TODO: i.e., the final counted_fruits_and_last_frame is a list, every element is: [[x,y,w,h],[array(xc,yc)]]
                counted_fruits_and_last_frame.append([bbox_pre_fr, [t.prev_pos]])

                h,w,_ = bbox_img_temp.shape
                # TODO: can remove this, no out-of-boundary index
                if not (h*w):
                    # print('h=%d w=%d' %(h,w), '\n')
                    continue
                # TODO:resize to the size COUNT_STD_SIZE
                bbox_img_temp = cv2.resize(bbox_img_temp, (128,128))
                frame_bbox_imgs.append(bbox_img_temp)
                # print ('bbox_img_shape', bbox_img_temp.shape, '\n')

                # TODO: visualize the source of the lost tracks (our counted bboxes) in current frame
                # cv2.imshow('bbox_img_temp',bbox_img_temp)
                # cv2.waitKey(100)
                # cv2.destroyAllWindows()
                # draw_bboxes(prev_bgr, bbox_pre_fr, color=Colors.green,
                #             thickness=3)
